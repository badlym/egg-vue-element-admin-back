...
class StudentrController extends Controller {
  ...
  async create() {
    const { ctx } = this;
    const res = await this.ctx.service.student.create();
    ctx.body = {
      code: 0,
      msg: '新增成功',
      data: res,
  ...
  async update() {
    ...
    // 校验参数
    // ctx.validate(this.StudentrUpdateTransfer)
    // 组装参数
    const { id } = ctx.params;
    console.log(id);
    const payload = ctx.request.body;
    // 调用 Service 进行业务处理
    const res = await service.student.update(id, payload);
    if (res[0] === 0) {
      ctx.status = 400;
  ...
  async show() {
    ...
    const { id } = ctx.params;
    // 调用 Service 进行业务处理
    const res = await service.student.show(id);
  ...
  async destroy() {
    ...
    const { id } = ctx.params;
    // 调用 Service 进行业务处理
    const res = await service.student.destroy(id);
    // 设置响应内容和响应状态码
